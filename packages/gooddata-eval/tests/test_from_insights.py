...
def test_a_pushpin_maps_size_and_colour_to_metrics():
    ...
    assert sorted(spec["metrics"]) == ["m_orders", "m_spend"]
    assert spec["segment_by"] == ["d_country_name"]
...
def test_a_bubbles_size_measure_is_kept_in_its_own_bucket():
    ...
    assert sorted(spec["metrics"]) == ["m_price", "m_volume"]
    assert spec["segment_by"] == ["m_profit"]
    assert spec["query"]["fields"]["m_profit"] == "metric/profit"
...
def test_a_sankeys_two_ends_are_dimensions():
    ...
    assert spec["metrics"] == ["m_orders"]
    assert sorted(spec["view_by"] + spec["segment_by"]) == ["d_customer_name", "d_product_name"]
...
def test_a_count_over_an_attribute_is_a_metric_the_scorer_can_compare():
    ...
    assert spec["metrics"] == ["m_visit_id"]
    assert spec["query"]["fields"]["m_visit_id"] == {"using": "attribute/visit_id", "aggregation": "COUNT"}
...
def test_a_real_sort_or_ranking_legitimises_a_ranking_title():
    ...
    assert sorted_spec["query"]["sort_by"] == [{"type": "attribute_sort", "by": "d_merchant_name", "direction": "ASC"}]
        ...
        (
            viz(
                ...
                [
                    {"localIdentifier": "measures", "items": [measure("m", "spend")]},
                    {"localIdentifier": "view", "items": [attribute("a", "merchant.NAME")]},
                    {"localIdentifier": "stack", "items": [attribute("b", "region.NAME")]},
                ],
            ),
...
def test_derived_variants_are_scorable_and_valid():
    base = convert(_bar("spend", "merchant.NAME"), DATE_IDS)
    envelope = build(derive(base, 3, DATE_IDS), "Show the top 3 Merchants by Spend", "d", set())
    assert _validation_errors(envelope) is None
...
def test_a_derived_item_records_where_it_came_from():
    base = convert(_bar("spend", "merchant.NAME"), DATE_IDS)
    envelope = build(derive(base, 5, DATE_IDS), "Show the top 5 Merchants by Spend", "d", set())
...
def test_derived_ranking_reads_as_a_ranking_not_a_breakdown():
    base = convert(_bar("spend", "merchant.NAME"), DATE_IDS)
    rules = _rules_for(derive(base, 3, DATE_IDS), DISPLAY)
...
def test_a_derived_question_naming_the_ranking_is_not_a_contradiction():
    spec = derive(convert(_bar("spend", "merchant.NAME"), DATE_IDS), 3, DATE_IDS)
...
def test_only_ranking_filters_are_derived():
    """A sort-only variant validates into a copy of its base, so none is produced."""
    specs = [convert(_bar("spend", "merchant.NAME"), DATE_IDS), convert(_bar("gross_revenue", "region.NAME"), DATE_IDS)]

    assert [p["_derived_kind"] for p in pick_derived(specs, DATE_IDS, 4, counts={})] == [
        "ranking_filter",
        "ranking_filter",
    ]
...
def test_rescued_items_are_spent_before_anything_the_generator_invents():
    ...
    # One base yields one variant, so a budget of 3 over one base is not filled.
    picked = pick_derived(specs, DATE_IDS, 3, counts={}, promised=promised)
    assert [p["_derived_basis"] for p in picked] == ["title", "shape"]
...
def test_a_bottom_ranking_keeps_the_lowest_rows():
    out = derive(convert(_bar("spend", "merchant.NAME"), DATE_IDS), 3, DATE_IDS, direction="bottom")
    assert list(out["query"]["filter_by"].values()) == [{"type": "ranking_filter", "using": "m_spend", "bottom": 3}]


def test_an_unknown_direction_is_a_programming_error():
    base = convert(_bar("spend", "merchant.NAME"), DATE_IDS)
    with pytest.raises(ValueError, match="direction"):
        derive(base, 3, DATE_IDS, direction="middle")
...
def test_dedup_compares_what_is_asked_not_how_it_is_titled():
    base = convert(_bar("spend", "merchant.NAME"), DATE_IDS)
    same = derive(base, 5, DATE_IDS)
    renamed = derive({**base, "title": "Something Else", "id": "v_other"}, 5, DATE_IDS)
    assert spec_signature(same) == spec_signature(renamed)

    other_n = derive(base, 3, DATE_IDS)
    other_end = derive(base, 5, DATE_IDS, direction="bottom")
...
def test_generate_derives_ranked_items_and_records_their_provenance(tmp_path):
    args = _args(
        tmp_path,
        snapshot_in=_snapshot_file(tmp_path, _snapshot(_bar("spend", "merchant.NAME"), _bar("revenue", "region.NAME"))),
        enrich_ranked=2,
    )
    assert generate(args) == 0
    ...
    assert {i["derived_kind"] for i in derived} == {"ranking_filter"}
