...
def _reject_unscorable(spec: dict) -> None:
    ...
    for item in spec["metrics"]:
        field = fields[item["field"] if isinstance(item, dict) else item]
        aggregated = False
...
def _rename_aliases(spec: dict) -> None:
    """Give every field a readable alias, in place.

    The convertor keeps AD's local identifiers, which in a real workspace are uuids
    (`d319bcb2d8c04442a684e3b3cd063381`). Nothing scores on the alias, but every skip
    reason and dry-run brief is read by a human.
    """
    fields = spec["query"]["fields"]
    taken: set = set()
    renamed = {}
    for alias, field in fields.items():
        uri = field["using"] if isinstance(field, dict) else field
        is_metric = uri.startswith(("metric/", "fact/")) or (isinstance(field, dict) and field.get("aggregation"))
        renamed[alias] = _alias("m_" if is_metric else "d_", uri, taken)
    spec["query"]["fields"] = {renamed[a]: f for a, f in fields.items()}
    for bucket in ("metrics", "view_by", "segment_by", "rows", "columns"):
        # A bucket item is an alias, or `{"field": alias, ...}` carrying `format`, `axis`,
        # `totals` or `display_as` -- presentation, which nothing scores. Keep the alias.
        spec[bucket] = [renamed.get(a["field"] if isinstance(a, dict) else a, a) for a in spec[bucket]]
    for entry in spec["query"]["filter_by"].values():
        for key in ("using", "attribute"):
            if isinstance(entry.get(key), str) and entry[key] in renamed:
                entry[key] = renamed[entry[key]]
    for entry in spec["query"]["sort_by"]:
        if entry.get("type") == "attribute_sort":
            entry["by"] = renamed.get(entry.get("by"), entry.get("by"))
        else:
            entry["metrics"] = [renamed.get(m, m) for m in entry.get("metrics") or []]
...
def classify(spec: dict, date_instance_ids: set) -> str:
    ...
    if any(resolve_alias_to_uri(a, fields).split("/", 1)[-1].split(".", 1)[0] in date_instance_ids for a in dims):
        return "time_series"
...
# so ranking coverage is near zero on most customer models: the eval can punish a
# spurious ranking but never confirm the agent builds a required one.
#
# A ranked variant is *derived*, not synthesized. Adding a LIMIT to a spec that already
# executes cannot make it unanswerable -- the LDM is untouched -- and "the top 3 X by Y"
# has exactly one correct spec, so a derived item is less ambiguous to grade than the
# insight it came from. What it loses is provenance: no human ever asked for it.
#
# Only a ranking filter is derived. A derived sort would be a weaker item than a derived
# ranking on the same base: "the top 3 X by Y" has one correct spec, while "X sorted by
# Y" leaves the direction to the reader, and a base that is worth ranking is worth
# ranking rather than merely ordering.
DERIVED_KIND = "ranking_filter"
...
def rankable(spec: dict, date_instance_ids: set) -> str | None:
    ...
    if len(dims) != 1 or ranks(spec):
        return None
    uri = resolve_alias_to_uri(dims[0], spec["query"]["fields"])
...
def derive(
    ...
    """A copy of `spec` keeping only the top or bottom `n` rows.
    ...
    out["_derived_kind"] = DERIVED_KIND
...
def rescued(promised: list, date_instance_ids: set, counts: dict | None = None) -> list:
    ...
    for error in promised:
        ...
        if alias is None:
            continue
        count = counts.get(resolve_alias_to_uri(alias, spec["query"]["fields"]))
        if error.n is None:
            n = derived_n(count)
        ...
        if n is None:
            continue
        out.append(derive(spec, n, date_instance_ids, error.direction, basis="title"))
    return out
...
def spec_signature(spec: dict) -> str:
    ...
    def resolve(value):
        if isinstance(value, str):
            return resolve_alias_to_uri(value, fields)
    ...
    return json.dumps(
        {
            ...
            "sorts": [resolve(entry) for entry in sorts_of(spec)],
        },
        sort_keys=True,
    )
...
def pick_derived(
    ...
    "Top Returned Reasons") come before the ones this generator invented. Within the
    invented ones, expanding every eligible insight would turn one popular metric into a
    third of the corpus and the pass rate into a measurement of one skill, so bases are
    taken round-robin by metric.
    """
    counts = counts or {}
    seen, out = set(), []
    ...
    for spec in specs:
        ...
        if alias is None:
            continue
        fields = spec["query"]["fields"]
        n = derived_n(counts.get(resolve_alias_to_uri(alias, fields)))
        if n is None:
            continue
        by_metric.setdefault(resolve_alias_to_uri(spec["metrics"][0], fields), []).append((spec, n))
    ...
    while queues and len(out) < limit:
        for queue in queues:
            if not queue:
                continue
            spec, n = queue.pop(0)
            if not take(derive(spec, n, date_instance_ids)):
                return out
...
def derived_candidates(specs: list, date_instance_ids: set, promised: list | None = None) -> set:
    ...
    for spec in list(specs) + [e.spec for e in promised or []]:
        ...
        if alias is not None:
            uris.add(resolve_alias_to_uri(alias, spec["query"]["fields"]))
    return uris
...
def generate(args, sdk_factory=None) -> int:
    ...
    if args.enrich_ranked:
        print(f"  derived from a base      {len(derived)}, {n_base} from real insights")
    ...
    if args.langfuse_out:
        origin = (
            ...
            + (
                f"; {len(derived)} item(s) derived from a base insight by adding a ranking filter (see `derived_from`)"
                if derived
                else ""
            )
        )
